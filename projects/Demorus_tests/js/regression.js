		...
		if (excCount == 5) {
			...
			$('#regression2').modal('show');


			var resultTime = $('#regression2 ul.results').find('li');
			for (let rt = 0; rt < resultTime.length; rt++) {
				...
				var first_dif = $('#regression2 ul.results li').eq(rt).html();
				var second_dif = $('#regression2 ul.results li').eq(rtt).html();
				...
				var difference = $('#regression2 ul.results li').eq(rt + 1).html() - $('#regression2 ul.results li').eq(rt).html();
			}
			...
			$('#regression2 .onboarding-content .reg_h41 span').html(result_percent_right);
			$('#regression2 .onboarding-content .reg_h42 span').html(middleNum + ':' + rand_ms);
			$('#regression2 .onboarding-content button').click(function () {
				location.href = 'schulte-5-5.html';
			})
